...
function createAppTimer (_, chunk) {
  ...
  var backendTimer = createAsyncTimer()
  var totalResults = {name: 'total', total: 0, app: 0, level: 0, chunk: 0}

  // Record time spent in 'put'
  var chunkPut = chunk.prototype.put
  chunk.prototype.put = function (n, buf, cb) {
    chunkTimer.start()
    backendTimer.start()
    chunkPut.call(this, n, buf, function (err, value) {
      chunkTimer.end()
      backendTimer.end()
      cb(err, value)
    })
  ...
  var chunkGet = chunk.prototype.get
  chunk.prototype.get = function (n, opts, cb) {
    ...
    chunkTimer.start()
    backendTimer.start()
    chunkGet.call(this, n, opts, function (err, value) {
      chunkTimer.end()
      backendTimer.end()
      cb(err, value)
    })
  ...
  // Record time spent in 'get'
  var levelupGet = levelup.prototype.get
  levelup.prototype.get = function (key_, options, callback) {
    ...
    levelTimer.start()
    backendTimer.start()
    levelupGet.call(this, key_, options, function (err, value) {
      levelTimer.end()
      backendTimer.end()
      callback(err, value)
    })
  ...
  // Record time spent in 'put'
  var levelupPut = levelup.prototype.put
  levelup.prototype.put = function (key_, value_, options, callback) {
    ...
    levelTimer.start()
    backendTimer.start()
    levelupPut.call(this, key_, value_, options, function (err, value) {
      levelTimer.end()
      backendTimer.end()
      callback(err, value)
    })
  ...
  var levelupBatch = levelup.prototype.batch
  levelup.prototype.batch = function (arr_, options, callback) {
    ...
    levelTimer.start()
    backendTimer.start()
    levelupBatch.call(this, arr_, options, function (err, value) {
      levelTimer.end()
      backendTimer.end()
      callback(err, value)
    })
  ...
  return {
    start: function (str) {
      ...
      var res = {
        ...
        app: (appEnd - appStart) - backendTimer.getTime(),
